...
when 'debian'
  ...
  apt_repository 'mongodb' do
    uri File.join(node[:mongodb][:source_root_url], "repo", node[:mongodb][:apt_repo])
    distribution 'dist'
    components ['10gen']
...
when 'rhel', 'fedora'
  yum_repository 'mongodb' do
    ...
    baseurl File.join(node[:mongodb][:source_root_url], "repo/redhat/os", node['kernel']['machine'] =~ /x86_64/ ? 'x86_64' : 'i686')
    action :create
    gpgcheck false
    enabled true
  end
